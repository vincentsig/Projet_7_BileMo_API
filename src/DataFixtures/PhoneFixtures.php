...
class PhoneFixtures extends Fixture
{
    public function load(ObjectManager $manager)
    {
        ...
        $phoneNames = call_user_func_array('array_merge', $this->getPhonelist());

        for ($i = 0; $i < count($phoneNames); $i++) {
            $phone = new Phone();

            $phone->setName($phoneNames[$i]);
            $phone->setBrand($this->getBrandName($phoneNames[$i], $this->getPhonelist()));
            ...
            $phone->setOS(($this->getListOs()[mt_rand(0, 3)]));
            $phone->setSim($this->getListSim()[mt_rand(0, 2)]);
    ...
    private function getPhonelist() : array
    {
        return [
            'Apple' => [
    ...
    private function getListOs() : array
    {
        return ['Android', 'iOS', 'Tizen', 'Linux',];
    }

    private function getListSim() : array
    {
        return ['Nano SIM', 'Micro SIM', 'Standart SIM'];
    }
}
